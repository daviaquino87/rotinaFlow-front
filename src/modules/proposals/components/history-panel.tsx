...
export function HistoryPanel({
  ...
  const renameMut = useMutation({
    ...
    onError: () =>
      toast({ title: t("components.historyPanel.renameError"), variant: "destructive" }),
  });
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-10 px-4 text-center">
        ...
        <p className="text-sm font-semibold text-slate-700">
          {t("components.historyPanel.emptyState.title")}
        </p>
        <p className="text-xs text-slate-500">
          {t("components.historyPanel.emptyState.description")}
        </p>
        <Link href="/routine">
          <Button variant="outline" className="text-xs h-8 px-3 gap-1.5">
            <CalendarDays className="w-3.5 h-3.5" />{" "}
            {t("components.historyPanel.emptyState.createButton")}
          </Button>
        </Link>
      </div>
    );
    ...
    <div className="flex flex-col">
      <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-2 bg-slate-50 sticky top-0">
        ...
        <h2 className="text-sm font-bold text-slate-700">
          {t("components.historyPanel.header.title")}
        </h2>
      ...
      <div className="divide-y divide-slate-100">
        {proposals.map((p, idx) => {
          ...
          const fallbackLabel = t("components.historyPanel.routineFallbackLabel", {
            number: proposals.length - idx,
          });
                  ...
                  {isEditing ? (
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        ...
                        title={t("components.historyPanel.save")}
                        aria-label={t("components.historyPanel.save")}
                      >
                        {isSaving ? (
                      ...
                      <button
                        ...
                        title={t("components.historyPanel.cancelEditTitle")}
                        aria-label={t("components.historyPanel.cancelEditAriaLabel")}
                      >
                    ...
                    <button
                      ...
                      title={t("components.historyPanel.renameHint")}
                    >
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  {isApproved ? (
                    <span className="flex items-center gap-1 text-[10px] font-semibold text-green-700 bg-green-50 border border-green-200 px-2 py-0.5 rounded-full">
                      ...
                      {t("components.historyPanel.status.approved")}
                    </span>
                  ) : (
                    <span className="text-[10px] font-semibold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">
                      {t("components.historyPanel.status.pending")}
                    </span>
                  )}
                  <button
                    ...
                    title={t("components.historyPanel.deleteRoutine")}
                    aria-label={t("components.historyPanel.deleteRoutine")}
                  >
                    {isDeleting ? (
                      <Loader2 className="w-3.5 h-3.5 animate-spin" />
                    ) : (
              ...
              <div className="flex gap-2">
                <Link href={`/proposal/${pUuid}`} className="flex-1">
                  <button className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 bg-white text-xs font-semibold text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all">
                    <Eye className="w-3.5 h-3.5" /> {t("components.historyPanel.viewButton")}
                  </button>
                </Link>
                <button
                ...
                >
                  ...
                  {isApproved
                    ? t("components.historyPanel.syncButton.resync")
                    : t("components.historyPanel.syncButton.sync")}
                </button>
              </div>
            </div>
          );
      ...
      <div className="px-4 py-3 border-t border-slate-100">
        <Link href="/routine">
          <button className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl border border-dashed border-slate-300 text-xs font-semibold text-slate-500 hover:border-primary hover:text-primary hover:bg-primary/5 transition-all">
            <CalendarDays className="w-3.5 h-3.5" /> {t("components.historyPanel.newRoutineButton")}
          </button>
        </Link>
      </div>
    </div>
